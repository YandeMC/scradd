...
export async function getTournamentData():Promise<Tournament> {
    return  await axios.get(
        `https://api.challonge.com/v1/tournaments/${TOURNAMENT_URL}.json`,
        {
...
export async function getParticipant(id: number) {
    ...
    return { ...(response.data as Participant).participant, discordId: regex.exec(response.data.participant.display_name)?.[1] as string }
}
export async function removeParticipant(name: string): Promise<boolean> {
    try {
        ...
        const regex = /^.*\((.+)\)$/
        const participant = participants.find(p => regex.exec(p.participant.display_name)?.[1] === compressId(name));
...
export async function findMatch(player1Name: string, player2Name: string): Promise<Match | false> {
    try {
        ...
        const regex = /^.*\((.+)\)$/
        const participants: Participant[] = participantsResponse.data;
        const player1 = participants.find(p => regex.exec(p.participant.display_name)?.[1] === compressId(player1Name));
        const player2 = participants.find(p => regex.exec(p.participant.display_name)?.[1] === compressId(player2Name));
        ...
        const match = matches.filter((m) => m.match.state == "open").find(
            m => (m.match.player1_id === player1.participant.id && m.match.player2_id === player2.participant.id) ||
                (m.match.player1_id === player2.participant.id && m.match.player2_id === player1.participant.id)
        );
...
export async function setMatchWinner(matchId: number, winnerId: number, scores: [number, number]): Promise<boolean> {
    try {
