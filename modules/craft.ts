...
import { ApplicationCommandOptionType, AutocompleteInteraction, ComponentType, TextInputStyle, type Snowflake } from "discord.js";
import { matchSorter } from "match-sorter";
import { defineSubcommands } from "strife.js";

export const elementDB = new Database<{
    name: string, id: string, recipie?: string, discoverer?: string
}>("elements");

await elementDB.init()
export const ownedElementDB = new Database<{
    user: Snowflake, elements: string
}>("playerelements");
                ...
                options: {
                    a: {
                        autocomplete(interaction: AutocompleteInteraction) {
                            return matchSorter(
                                elementDB.data.filter((e) => getOwnerships(interaction.user.id).includes(e.name)),
    ...
    async (i, o) => {
        ...
        switch (sbcmd) {
            case "combine": {
                ...
                const a = o.options.a
                const b = o.options.b
                const crafted = getElements().find((element) => (element.recipie?.[0] == a && element.recipie?.[1] == b) || (element.recipie?.[0] == b && element.recipie?.[1] == a))
                if (!crafted) {
                    await i.showModal({
                    ...
                    elementDB.data = [
                        ...elementDB.data,
                        { name: submit.fields.getTextInputValue("name"), id: elementDB.data.reduce((p, c): any => { return (+p < +c.id ? c.id : p) }, "0"), recipie: JSON.stringify([a, b]) }
                    ]

                }
                if (crafted)
                    addOwnership(i.user.id, crafted?.name ?? "")
...
function getElements() {
    return elementDB.data.map((e) => ({ ...e, recipie: (JSON.parse(e.recipie ?? "[]") as [string, string] | undefined) }))
}

function getOwnerships(id: string) {
    return ["air", "fire", "water", "earth", ...(ownedElementDB.data.find((e) => e.user == id)?.elements.split("/") ?? [])]
}
...
function addOwnership(id: string, element: string) {
    ...
    if (index == -1)
        data.push({ user: id, elements: [element].join("/") })
    else
        data[index] = { user: id, elements: [...(data[index]?.elements.split("/") ?? []), element].join("/") }
