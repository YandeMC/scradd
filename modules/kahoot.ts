...
import { ApplicationCommandOptionType, ButtonInteraction, ButtonStyle, ComponentType, TimestampStyles, time } from "discord.js";
import { NodeHtmlMarkdown } from "node-html-markdown";
import { defineChatCommand } from "strife.js";
const msgses = ["Feeling Competitive?",
    "Lightning Smart?",
    "Easy Does It!",
    "Solid Effort!"]

const kahootEmojis = {
    array: ["<:khred:1247304316251934791>", "<:khblue:1247304313781354628>", "<:khyellow:1247304314733465683>", "<:khgreen:1247304312523198485>"],
    ...
    async (i, o) => {
        ...
        const message = await i.reply({
            ...
            embeds: [
                {
                    title: kahoot.title,
                    description: "Questions:\n" + kahoot.questions.map((q) => NodeHtmlMarkdown.translate(q.question)).join("\n\n"),
                    fields: []
                }
            ],
            components: [
        ...
        const startbtn = await message.awaitMessageComponent({
        ...
        message.createMessageComponentCollector({
            ...
            for (const q of kahoot.questions) {
                ...
                let answerd: { [id: string]: { interaction: ButtonInteraction, choice: boolean, responseTime: number } } = {}
                ...
                const p = new Promise(async (resolve) => {
                    ...
                    collector.on("collect", async (button) => {
                        if (!players[button.user.id]) return await button.reply({ content: "You Arent In This Game!", ephemeral: true })
                        if (answerd[button.user.id]) {
                            await button.deferUpdate();
                        } else {
                            await button.reply({ ephemeral: true, fetchReply: true, content: msgses.at(Math.floor(Math.random() * msgses.length)) })
                            answerd[button.user.id] = { choice: q.choices[kahootEmojis.names.indexOf(button.customId)]?.correct || false, interaction: button, responseTime: Date.now() - now };
                ...
                for (const key in answerd) {
                    const a = answerd[key]
                    if (a) {
                        const pPoints = Math.round((1 - ((a.responseTime / q.time) / 2)) * 1000 * q.pointsMultiplier)
                        ...
                        await a.interaction.editReply(`${answerd[key]?.choice ? "Correct!" : "Wrong!"} +${pPoints} Points`)
                    }
                }
                await message.edit({
                    embeds: [
                ...
                await message.edit({
                    embeds: [
                        {
                            title: "Scoreboard",
                            description: `${Object.entries(players).toSorted((a, b) => b[1].points - a[1].points).map((p, i) => `${i + 1}<@${p[0]}> ${p[1].points}`).slice(0, 5).join("\n")}`
                        }
                    ],
                    components: [
                        {
            ...
            await message.edit({
                embeds: [
                    {
                        title: "Podium",
                        description: `${Object.entries(players).toSorted((a, b) => b[1].points - a[1].points).map((p, i) => `${i <= 2 ? "## " : ""}${i + 1}<@${p[0]}> ${p[1].points} Points`).slice(0, 5).join("\n")}`
                    }
                ],
                components: [
