import { ButtonStyle, ComponentType, type ButtonInteraction, type RepliableInteraction, type User } from "discord.js";
...
class ShopItem {
  ...
  constructor(name: string, description: string, price: number, buyFunction: (user: User) => Promise<number> | number) {
    this.name = name;
...
let shopItems = [
  new ShopItem(
    ...
    async function (user) {
      ...
      await u.roles.add(role).catch((e) => { log(`${constants.emojis.statuses.no} ${e}`, LogSeverity.Alert); return -3 })
      return 1
    }
  ),
  new ShopItem(
  ...
  await interaction.reply({
    embeds: [
      {
        ...
        fields: [
          ...shopItems.map((item) => { return { name: `${item.name} (${item.price} 🧽)`, value: item.description } })
        ]
    ...
    components: chunkArray(shopItems, 5).map((chunk: any[]) => ({
      ...
      components: chunk.map((item: { name: any; }) => ({
        ...
        label: item.name,
        style: [ButtonStyle.Danger, ButtonStyle.Primary, ButtonStyle.Secondary, ButtonStyle.Success].at(Math.floor(Math.random() * 4)) as any,
      })),
    }))
  })
}
...
export async function buyItem(button: ButtonInteraction) {
  ...
  const ans = await message
    ...
    ans.deferUpdate()
  if (ans.customId == "decline") return button.editReply({ content: `${constants.emojis.statuses.yes} Canceled Purchase` })
   await button.editReply({ content: `${constants.emojis.statuses.yes} Attempting Purchase` })
  const result = await item.buy(button.user)
  switch (result) {
    case 1: {
      return await button.editReply({ content: `${constants.emojis.statuses.yes} Purchase Successful` })
    }
    case 0: {
      return await button.editReply({ content: `${constants.emojis.statuses.no} You already have the thing` })
    }
    case -1: {
      return await button.editReply({ content: `${constants.emojis.statuses.no} You dont have enough money` })
    }
    case -2: {
      return await button.editReply({ content: `${constants.emojis.statuses.no} item does not exist` })
    }
    case -3: {
      return await button.editReply({ content: `${constants.emojis.statuses.no} general error happened, it has been reported` })
    }
  }
}