...
export async function pay(interaction: RepliableInteraction, user: User, amount: number) {
	if (0 >= amount)
		return await interaction.reply({
			ephemeral: true,
			content: "please put in a number more than 0",
		});
	if (user.id == interaction.user.id)
		return await interaction.reply({ ephemeral: true, content: "cant pay yourself sorry" });
	if (getSponges(interaction.user) < amount)
		return await interaction.reply({ ephemeral: true, content: "u too poor" });
	...
	switch (ans.customId) {
		case "confirm": {
			...
			await user
				.send(`<@${interaction.user.id}> paid you ${amount} 🧽`)
				.catch(() =>
					interaction.followUp({ ephemeral: true, content: "DM failed to send" }),
				);
