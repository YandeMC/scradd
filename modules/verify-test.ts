...
if (process.env.SCRATCH_PASS) {
    ...
    interface res { "publicCode": string; "privateCode": string }

    async function testProject() {
        const res = await fetch("https://scratch-coders-auth-server.vercel.app/auth/gettokens?redirect=aa&method=comment")
        const json: res = await res.json()
        await project.comment(json.publicCode)
        // await wait(2000)
        const res2 = await fetch("https://scratch-coders-auth-server.vercel.app/auth/verifytoken/" + json.privateCode)
        const json2: { valid: boolean } = await res2.json()
        return { valid: json2.valid, type: "project" }

    }
    async function testComment() {
        const res = await fetch("https://scratch-coders-auth-server.vercel.app/auth/gettokens?redirect=aa&method=profile-comment&username=yandetest")
        const json: res = await res.json()
    ...
    async function TestCloud() {
        const res = await fetch("https://scratch-coders-auth-server.vercel.app/auth/gettokens?redirect=aa&method=cloud")
        const json: res = await res.json()
    ...
    function getStatus(status?: string) {
        ...
        return status == "wait" ? `${statuses.wait} In Progress` : status == "true" ? `${statuses.true} Success` : `${statuses.false} Failed`
    }
}