...
class DealOrNoDeal {
    ...
    initializeCases(): { amount: number, isOpen: boolean }[] {
        const amounts: number[] = [0.01, 1, 5, 10, 25, 50, 75, 100, 200, 300, 400, 500, 750,
    ...
    casesUntilDealerCalls() {
        ...
        this.casesUntilCalls = ((calls.find((num) => num >= this.casesOpened) ?? 0) - this.casesOpened)
    ...
    calculateBankersOffer() {
        ...
        const E = (this.cases.filter((cas) => !cas.isOpen).map((ca) => ca.amount).reduce((p, c) => p + c)) / C
        const M = this.cases.filter((cas) => !cas.isOpen).reduce((p, c) => p > c.amount ? p : c.amount, 0)

        const offer = 12275.30 +
...
export default async function dond(i: RepliableInteraction) {
    ...
    const message = await i.reply({
        components: components(game), embeds: embed(game)
    })
...
function components(game: DealOrNoDeal): any {
    if (game.casesUntilCalls == 0) {
        ...
        if (game.chosenCase == -1) {
            ...
            return chunkArray(game.cases.filter((_: any, index: any) => index != game.chosenCase), 5).map((chunk: any[]) => ({
                type: ComponentType.ActionRow,
    ...
    return ([
        {
            ...
            description: `Your chosen case: ${game.chosenCase + 1}\n${c?.dealerCall ?
                ...
                game.turn == 1 ? "# 📞 Its the dealer" : `# 📞 The number went ${game.lastOffer > c.dealerCall.dealerOffer ? "down" : "up"}`,
                `# Dealers Offer: $${c.dealerCall.dealerOffer}`].at(callStage)
                :
