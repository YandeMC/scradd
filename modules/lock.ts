...
defineChatCommand(
	...
	async (i, options) => {
		if (!i.memberPermissions.has("ManageChannels"))
			return i.reply({ ephemeral: true, content: "You Do Not Have The Permissions" });
		const date = options.duration ? parseTime(options.duration) : null;
		if (date)
			if (+date < Date.now() + 3_000 || +date > Date.now() + 60 * 60 * 24 * 2000) {
		...
		if (channel.isThread())
			return await i.reply({
				ephemeral: true,
				content:
					"you cannot lock a thread using this command, use the right click menu instead.",
			});
		if (!config.roles.verifiedPerms)
			return i.reply({ ephemeral: true, content: "Verified Perms Role Does Not Exist." });
		...
		await i.reply(
			`${constants.emojis.statuses.yes} ${talkPerms ? "Locked" : "Unlocked"} ${channel.toString()}${
				date ? ` until ${time(Math.floor(+date / 1000), TimestampStyles.RelativeTime)}` : ""
			}`,
		);
	},
);
