...
defineSubcommands(
	...
	async (i, o) => {
		...
		switch (sbcmd) {
			case "add-child": {
				if (!i.memberPermissions.has("ManageRoles"))
					return await i.reply({ ephemeral: true, content: `# NO PERMS!!! GET OUT!!!` });
				addChild(o.options["parent-role"].id, o.options["child-role"].id);
				await i.reply({
					allowedMentions: {},
					content: `Added ${o.options["child-role"].toString()} as a child of ${o.options["parent-role"]}`,
				});
				return;
			}
			case "remove-child": {
				...
				await i.reply({
					allowedMentions: {},
					content: `Removed ${o.options["child-role"].toString()} as a child of ${o.options["parent-role"]}`,
				});
				return;
			}
			case "list": {
				i.reply({
					content: roleCategoryDB.data
						.map(
							(r) =>
								`${`<@&${r.categoryid}>`}:\n   ${r.children?.split("/").map((r) => `<@&${r}>`)}`,
						)
						.join("\n\n"),
					allowedMentions: {},
				});
			...
			case "sync": {
				...
				await msg.edit(
					"Syncing Categories\n`" +
						createProgressBar(50, 0) +
						` ${Math.round((0 / members.length) * 1000) / 10}% (${0}/${members.length})\``,
				);
				for (let index = 0; index < members.length; index++) {
					const member = members[index];
					if (member) await updateCategories(member);
					if (lastStep < index) {
						lastStep += Math.floor(members.length / steps);

						await i.editReply(
							"Syncing Categories\n`" +
								createProgressBar(50, index / members.length) +
								` ${Math.round((index / members.length) * 1000) / 10}% (${index}/${members.length})\``,
						);
					}
				}
				await msg.edit(
...
function addChild(id: string, element: string) {
	...
	if (index == -1) data.push({ categoryid: id, children: [element].join("/") });
	else
		data[index] = {
			categoryid: id,
			children: [...(data[index]?.children.split("/") ?? []), element].join("/"),
		};
...
function removeChild(id: string, element: string) {
	...
	if (index == -1) data.push({ categoryid: id, children: [element].join("/") });
		...
		data[index] = {
			categoryid: id,
			children: [...(data[index]?.children?.split("/") ?? [])]
				.filter((r) => r != element)
				.join("/"),
		};
