...
defineEvent("messageCreate", async (message) => {
    if (
        ...
        return;
    if (!message.channel.appliedTags.map(getTag).find((t) => t?.name.toLowerCase() === "code help needed")) return
    const msgrole = message.channel.appliedTags.map(getTag).filter((r) => r != undefined)?.map((tag) => roles[tagMap[tag?.name.toLowerCase() ?? ""] ?? ""]).filter((role): role is Role => role !== undefined);
    let msg = []
...
function pickMembers(array: GuildMember[], x: number) {
    ...
    const shuffledArray = array.filter((u) => u.presence?.status !== "offline").sort(() => Math.random() - 0.5);
