...
const files = readdirSync(path.join(__dirname, mp3Directory)).filter(file => file.endsWith('.mp3'));
    ...
    connection.on(VoiceConnectionStatus.Ready, async () => {
        ...
        try {
            ...
            function playNext() {
                ...
                currentIndex = (currentIndex + 1 + Math.floor(Math.random() * (files.length - 2))) % files.length;
        ...
        async function updateMsg(i: number) {
            ...
            await messgae?.edit({
                embeds: [{
                    title: "Music Player",
                    description: files
                        .map((file, index) => `${index == i ? "<:green:1196987578881150976>" : "<:TTT:1204592236407558154>"} ${file.replace(".mp3", "").replace("_", " ").replace("-", " ")}`)
                        .join("\n")
                }]
            })
        }
