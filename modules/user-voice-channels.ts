...
Promise.all([...((await config.guild.channels.fetch()).values())].map((c) => c?.name.startsWith(userChannelPrefix) && c.type == ChannelType.GuildVoice ? c.delete() : null))
        ...
        if (usrCh) {
            if (usrCh.id != channel?.id) {
                ...
                if ([...usrCh.members.values()].length === 0) {
                    ...
                    if (!theChosenOne) usrCh.delete(); else {
                        userChannels[theChosenOne.id] = usrCh
                        usrCh.setName(`${userChannelPrefix} ${theChosenOne.displayName}`, "User Channel Ownership Changed")
                    }
                }
            }
        }
