...
export const badWordRegexps = badWords.map(
	([strings = [], words = [], prefixes = []]) =>
		new RegExp(
			...
			`\\b(?:${words.length ? `(?:${decodeRegexps(words, false)})\\b` : "(?!x)x"}${prefixes.length ? `|${decodeRegexps(prefixes, false)}` : "(?!x)x"
			})`,
			regexpFlags,
		),
);
...
export default function tryCensor(
	text: string,
	strikeShift = 0,
): false | { warning: boolean, censored: string; strikes: number; words: string[][]; regexps: RegExp[][] } {
	let words: string[][] = [];
	let regexps: RegExp[][] = [];
