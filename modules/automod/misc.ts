...
export const badWordRegexps = badWords.map(
	([strings = [], words = [], prefixes = []]) =>
		new RegExp(
			(strings.length ? `${decodeRegexps(strings, false)}|` : "(?!x)x") +
				`\\b(?:${words.length ? `(?:${decodeRegexps(words, false)})\\b` : "(?!x)x"}${
					prefixes.length ? `|${decodeRegexps(prefixes, false)}` : "(?!x)x"
				})`,
			regexpFlags,
		),
);
export const warningRegexps = badWords.map(
	([strings = [], words = [], prefixes = []]) =>
		new RegExp(
			(
				(strings.length ? `${decodeRegexps(strings, true)}|` : "(?!x)x") +
				`\\b(?:${words.length ? `(?:${decodeRegexps(words, true)})\\b` : "(?!x)x"}${
					prefixes.length ? `|${decodeRegexps(prefixes, true)}` : "(?!x)x"
				})`
			)
				.replace(/\.\{0,2}\+/g, "+.{0,2}")
				.replace(/\.\{0,2}\*/g, "*.{0,2}")
				.replace(/\.\{0,2}\?/g, "?.{0,2}")
				.replace(/\.\{0,2}\|/g, "|")
				.replace(/\.\{0,2}\{(\d+)\,\}/g, "{$1,}.{0,2}"),
			regexpFlags,
		),
);

function decodeRegexps(regexps: RegExp[], hasInbetween: boolean): string {
...
export function decodeRegexp({ source }: RegExp, hasInbetween: boolean): string {
	...
	return caesar(source).replaceAll(
		...
		(letter) =>
			...
			}[letter] || letter) + inbetweenCharMatch),
	);
}
...
export default function tryCensor(
	...
	words = [];
	regexps = [];
	const warned = warningRegexps.reduce((string, regexp, index) => {
		words[index] ??= [];
		regexps[index] ??= [];
		return string.replaceAll(regexp, (word) => {
			if (
