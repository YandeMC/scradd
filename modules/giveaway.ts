...
defineChatCommand(
    {
        name: "giveaway", description: "start a giveaway", access: false, options: {
            item: {
    ...
    async (int, options) => {
        ...
        if (options.emoji === BOARD_EMOJI) return await int.reply({
            ephemeral: true,
        ...
        try { await message.react(options.emoji ?? "🎉") } catch (e: any) {
            if (e.rawError.message == "Unknown Emoji") return await message.edit({ content: "Unknown Emoji", embeds: [] }); else
                return await message.edit({ content: "an error occured", embeds: [] })
